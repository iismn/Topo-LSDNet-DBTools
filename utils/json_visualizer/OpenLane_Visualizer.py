...
def _draw_lane_segment(ax, lane_segment, lane_index, with_centerline, with_laneline, with_id=False, with_endpoints=False):
    ...
    if with_laneline:
        if lane_segment['left_laneline'] and _is_in_bev_range(lane_segment['left_laneline']):
            has_visible_parts = True
            _draw_line(ax, {
                'points': lane_segment['left_laneline'], 
                'linetype': lane_segment['left_laneline_type']
            }, with_endpoints=with_endpoints, lane_id=lane_index, line_type='left', with_id=with_id)
        
        if lane_segment['right_laneline'] and _is_in_bev_range(lane_segment['right_laneline']):
            has_visible_parts = True
            _draw_line(ax, {
                'points': lane_segment['right_laneline'], 
                'linetype': lane_segment['right_laneline_type']
...
def visualize_openlane_json(json_path, output_path=None, with_centerline=True, with_laneline=True, 
    ...
    # Count visible lane segments manually
    visible_lanes = 0
    for lane_segment in annotation['lane_segment']:
        if (lane_segment['centerline'] and _is_in_bev_range(lane_segment['centerline'])) or \
           (lane_segment['left_laneline'] and _is_in_bev_range(lane_segment['left_laneline'])) or \
           (lane_segment['right_laneline'] and _is_in_bev_range(lane_segment['right_laneline'])):
            visible_lanes += 1
